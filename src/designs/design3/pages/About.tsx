    ...
    <div className="min-h-screen bg-ivory">
      ...
      <section ref={heroRef} className="relative py-24 bg-forest overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <Breadcrumb items={[{ label: 'About Us' }]} className="mb-8 text-ivory/60" />
          <div className={`text-center transform transition-all duration-700 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <p className="text-copper font-serif tracking-widest uppercase mb-4">Established 1992</p>
            <h1 className="font-cormorant font-bold text-4xl sm:text-5xl md:text-6xl text-ivory mb-6">Our Story</h1>
            <OrnamentDivider variant="ornate" color="copper" className="max-w-md mx-auto" />
            <p className="font-serif text-ivory/80 max-w-2xl mx-auto mt-6 text-lg leading-relaxed">For over three decades, Advanced Appliance Repair Service has been the trusted choice for discerning New Jersey homeowners.</p>
          </div>
        </div>
      </section>

      <section ref={storyRef} className="py-20 bg-ivory">
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center transform transition-all duration-700 ${storyVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="relative">
              <div className="relative aspect-[4/5] max-w-md mx-auto">
                <div className="absolute inset-4 border-2 border-copper/30" />
                ...
                <div className="absolute top-0 left-0 w-12 h-12 border-t-4 border-l-4 border-copper z-20" />
                <div className="absolute top-0 right-0 w-12 h-12 border-t-4 border-r-4 border-copper z-20" />
                <div className="absolute bottom-0 left-0 w-12 h-12 border-b-4 border-l-4 border-copper z-20" />
                <div className="absolute bottom-0 right-0 w-12 h-12 border-b-4 border-r-4 border-copper z-20" />
              </div>
              <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 bg-burgundy text-ivory px-6 py-2 rounded-full shadow-lg z-30"><p className="font-serif text-sm">Founder & Master Technician</p></div>
            </div>
            <div>
              <p className="text-burgundy font-serif tracking-widest uppercase mb-4">The Founder's Vision</p>
              <h2 className="font-cormorant font-bold text-4xl text-forest mb-6">A Legacy of Excellence</h2>
              <OrnamentDivider variant="simple" color="forest" className="max-w-xs mb-8" />
              <div className="space-y-6 font-serif text-forest/80 leading-relaxed">
              ...
              <blockquote className="mt-8 pl-6 border-l-4 border-copper"><p className="font-cormorant text-2xl text-forest italic">"Every repair is an opportunity to earn a customer for life."</p><cite className="font-serif text-burgundy mt-2 block">- Robert Mitchell, Founder</cite></blockquote>
            </div>
          </div>
        </div>
      </section>

      <section ref={valuesRef} className="py-20 bg-forest/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-16 transform transition-all duration-700 ${valuesVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <p className="text-burgundy font-serif tracking-widest uppercase mb-4">What Guides Us</p>
            <h2 className="font-cormorant font-bold text-4xl text-forest mb-4">Our Core Values</h2>
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 transform transition-all duration-700 delay-200 ${valuesVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            {values.map((value, index) => (
              <div key={value.title} className="bg-ivory rounded-xl p-8 border-2 border-forest/10 hover:border-copper hover:shadow-xl transition-all text-center group" style={{ transitionDelay: `${index * 100}ms` }}>
                <div className="w-16 h-16 mx-auto mb-6 rounded-full bg-burgundy/10 flex items-center justify-center text-burgundy group-hover:bg-burgundy group-hover:text-ivory transition-colors">{value.icon}</div>
                <h3 className="font-cormorant font-bold text-xl text-forest mb-3">{value.title}</h3>
                <p className="font-serif text-forest/70 text-sm leading-relaxed">{value.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={teamRef} className="py-20 bg-forest text-ivory">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-16 transform transition-all duration-700 ${teamVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <p className="text-copper font-serif tracking-widest uppercase mb-4">Meet the Team</p>
            <h2 className="font-cormorant font-bold text-4xl text-ivory mb-4">Our Leadership</h2>
            <OrnamentDivider variant="floral" color="copper" className="max-w-md mx-auto" />
          </div>
            ...
            {teamMembers.map((member, index) => (
              <div key={member.id} className="text-center group" style={{ transitionDelay: `${index * 150}ms` }}>
                <div className="relative mb-6 inline-block">
                  <div className="w-48 h-48 mx-auto rounded-full overflow-hidden border-4 border-copper shadow-xl">
                    <img src={member.image} alt={member.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  </div>
                </div>
                <h3 className="font-cormorant font-bold text-2xl text-ivory mb-1">{member.name}</h3>
                <p className="text-copper font-serif mb-4">{member.role}</p>
                <p className="font-serif text-ivory/70 text-sm leading-relaxed max-w-xs mx-auto">{member.bio}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={awardsRef} className="py-20 bg-ivory">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-16 transform transition-all duration-700 ${awardsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <p className="text-burgundy font-serif tracking-widest uppercase mb-4">Recognition</p>
            <h2 className="font-cormorant font-bold text-4xl text-forest mb-4">Awards & Certifications</h2>
            <OrnamentDivider variant="ornate" color="burgundy" className="max-w-md mx-auto" />
          </div>
      ...
      <section className="py-20 bg-burgundy text-ivory">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="mb-8"><svg className="w-16 h-16 mx-auto text-copper" fill="currentColor" viewBox="0 0 24 24"><path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" /></svg></div>
          <h2 className="font-cormorant font-bold text-4xl sm:text-5xl text-ivory mb-6">Our Commitment</h2>
          <p className="font-serif text-ivory/90 text-xl leading-relaxed mb-8 max-w-3xl mx-auto">We pledge to provide courteous, professional, fast, and reliable appliance repair services.</p>
          <div className="flex flex-wrap justify-center gap-4">
            {['Licensed', 'Bonded', 'Insured', 'Factory Authorized'].map((item) => (<span key={item} className="px-6 py-2 bg-ivory/10 border border-copper rounded-full font-serif">{item}</span>))}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-forest text-ivory">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="font-cormorant font-bold text-4xl text-ivory mb-6">Experience the Difference</h2>
          <p className="font-serif text-ivory/90 text-lg mb-8">Join thousands of satisfied customers who trust Advanced Appliance.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" className="bg-copper text-ivory hover:bg-copper/80 border-copper" href="/contact">Schedule Service</Button>
