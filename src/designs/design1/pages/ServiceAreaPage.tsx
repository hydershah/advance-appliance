'use client';

import React from 'react';
import Link from 'next/link';
import Image from 'next/image';
import {
  Header,
  Footer,
  Hero,
  SectionHeading,
  CTAButton,
  FAQAccordion,
  TestimonialCard,
  LocalBusinessSchema,
  BreadcrumbSchema,
} from '../components';
import { businessInfo, services, brands, testimonials, images } from '../data/content';
import { areaEnrichment } from '../data/areaContent';
import {
  buildServiceAreaBody,
  buildServiceAreaFaqs,
  serviceAreaCombos,
  getServiceForCombo,
  getAreaForServiceCombo,
  type ServiceAreaCombo,
} from '../data/serviceAreaCombos';
import { brandAreaCombos } from '../data/brandAreaCombos';

interface ServiceAreaPageProps {
  combo: ServiceAreaCombo;
}

const ServiceAreaPage: React.FC<ServiceAreaPageProps> = ({ combo }) => {
  const service = getServiceForCombo(combo);
  const area = getAreaForServiceCombo(combo);
  const ae = areaEnrichment[combo.areaSlug];
  const body = buildServiceAreaBody(combo);
  const faqs = buildServiceAreaFaqs(combo);

  if (!service || !area || !ae || !body) return null;

  const cityHits = testimonials.filter((t) =>
    t.location.toLowerCase().includes(area.name.toLowerCase()),
  );

  // Same-service nearby-area combos
  const nearbySameService = serviceAreaCombos
    .filter((c) => c.serviceSlug === combo.serviceSlug && c.slug !== combo.slug)
    .filter((c) => ae.nearbyAreaSlugs.includes(c.areaSlug))
    .slice(0, 3);

  // For this area, brand combos that exist (cross-link to brand×area combos)
  const brandLinksInArea = brandAreaCombos.filter((c) => c.areaSlug === combo.areaSlug);

  const serviceShort = service.name.toLowerCase();

  const breadcrumbs = [
    { name: 'Home', url: '/' },
    { name: 'Services', url: '/services' },
    { name: service.name, url: `/services/${service.slug}` },
    { name: `${area.name}, NJ`, url: `/${combo.slug}` },
  ];

  return (
    <>
      <LocalBusinessSchema />
      <BreadcrumbSchema items={breadcrumbs} />
      <Header />
      <main>
        <Hero
          title={`${service.name} in ${area.name}, NJ`}
          subtitle={`Same-Day Service Available`}
          description={`Factory-trained technicians, OEM parts, and 30+ years of ${serviceShort} experience across ${area.name} — ${area.zipCodes.join(', ')}.`}
          image={service.image || images.hero}
          showCTA={true}
          overlay="gradient"
          height="medium"
          align="left"
        />

        {/* Breadcrumbs */}
        <div className="bg-gray-50 py-4 border-b border-gray-100">
          <div className="container mx-auto px-6">
            <nav className="flex items-center space-x-2 text-sm">
              <Link href="/" className="text-gray-500 hover:text-[#D4AF37]">
                Home
              </Link>
              <span className="text-gray-300">/</span>
              <Link
                href={`/services/${service.slug}`}
                className="text-gray-500 hover:text-[#D4AF37]"
              >
                {service.name}
              </Link>
              <span className="text-gray-300">/</span>
              <span className="text-[#D4AF37]">{area.name}, NJ</span>
            </nav>
          </div>
        </div>

        {/* Intro */}
        <section className="py-24 lg:py-32 bg-white">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <div>
                <SectionHeading
                  subtitle={`${area.county} County, ${area.state}`}
                  title={`${service.name} Specialists in ${area.name}`}
                  align="left"
                />
                <div className="space-y-6 mt-8">
                  {body.introParas.map((p, i) => (
                    <p key={i} className="text-gray-600 leading-relaxed">
                      {p}
                    </p>
                  ))}
                </div>
                <div className="grid grid-cols-3 gap-6 pt-8 border-t border-gray-100 mt-8">
                  <div className="text-center">
                    <div className="font-serif text-3xl text-[#D4AF37]">30+</div>
                    <div className="text-gray-500 text-xs uppercase tracking-wider mt-1">
                      Years in {area.county} Co.
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="font-serif text-3xl text-[#D4AF37]">$100</div>
                    <div className="text-gray-500 text-xs uppercase tracking-wider mt-1">
                      Diagnostic
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="font-serif text-3xl text-[#D4AF37]">4.9</div>
                    <div className="text-gray-500 text-xs uppercase tracking-wider mt-1">
                      Star Rating
                    </div>
                  </div>
                </div>
                <div className="mt-10 flex flex-col sm:flex-row gap-4">
                  <CTAButton
                    href={`tel:${businessInfo.phone.replace(/[^0-9]/g, '')}`}
                    variant="primary"
                    size="lg"
                    icon="phone"
                  >
                    Call {businessInfo.phone}
                  </CTAButton>
                  <CTAButton href="/contact" variant="outline" size="lg" icon="arrow">
                    Schedule Online
                  </CTAButton>
                </div>
              </div>
              <div className="relative">
                <div className="aspect-square overflow-hidden relative">
                  <Image
                    src={service.image || images.kitchen}
                    alt={`${service.name} technician serving ${area.name}, NJ`}
                    fill
                    className="object-cover"
                    sizes="(max-width: 1024px) 100vw, 50vw"
                  />
                </div>
                <div className="absolute -bottom-8 -left-8 w-48 h-48 border border-[#D4AF37] hidden lg:block" />
              </div>
            </div>
          </div>
        </section>

        {/* Common problems */}
        <section className="py-24 lg:py-32 bg-gray-50">
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto">
              <SectionHeading
                subtitle="Local Service Patterns"
                title={`Common ${service.name} Issues in ${area.name} Homes`}
                align="center"
              />
              <div className="space-y-8 mt-12">
                {body.problemParas.map((p) => (
                  <div key={p.title} className="border-l-2 border-[#D4AF37] pl-6">
                    <h3 className="font-serif text-xl text-black mb-3">{p.title}</h3>
                    <p className="text-gray-600 leading-relaxed">{p.description}</p>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </section>

        {/* Brands serviced */}
        <section className="py-20 bg-white border-y border-gray-100">
          <div className="container mx-auto px-6">
            <p className="text-center text-xs uppercase tracking-[0.3em] text-gray-500 mb-8">
              {service.name} Brands We Service in {area.name}
            </p>
            <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-6">
              {brands.slice(0, 12).map((brand, i) => (
                <span
                  key={i}
                  className="text-gray-400 text-lg font-serif hover:text-[#D4AF37] transition-colors cursor-default"
                >
                  {brand.name}
                </span>
              ))}
            </div>
            {/* Cross-link to enriched brand × area combos for this same area */}
            {brandLinksInArea.length > 0 && (
              <div className="max-w-5xl mx-auto mt-12">
                <p className="text-center text-sm text-gray-500 mb-6">
                  Premium brand specialty pages for {area.name}:
                </p>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  {brandLinksInArea.map((c) => {
                    const b = brands.find((br) => br.slug === c.brandSlug);
                    if (!b) return null;
                    return (
                      <Link
                        key={c.slug}
                        href={`/${c.slug}`}
                        className="text-center p-3 border border-gray-200 hover:border-[#D4AF37] transition-colors text-sm text-gray-700 hover:text-[#D4AF37]"
                      >
                        {b.name}
                      </Link>
                    );
                  })}
                </div>
              </div>
            )}
          </div>
        </section>

        {/* Why us */}
        <section className="py-24 lg:py-32 bg-white">
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto">
              <SectionHeading
                subtitle="Why Choose Us"
                title={`Why ${area.name} Homeowners Choose Us for ${service.name}`}
                align="center"
              />
              <div className="space-y-6 mt-12">
                {body.whyUsParas.map((p, i) => (
                  <p key={i} className="text-gray-600 leading-relaxed text-lg">
                    {p}
                  </p>
                ))}
              </div>
            </div>
          </div>
        </section>

        {/* Service coverage */}
        <section className="py-20 bg-gray-50 border-y border-gray-100">
          <div className="container mx-auto px-6">
            <div className="max-w-5xl mx-auto">
              <SectionHeading
                subtitle="Service Coverage"
                title={`${service.name} Across ${area.name}`}
                align="center"
              />
              <div className="grid md:grid-cols-2 gap-12 mt-12">
                <div>
                  <h3 className="font-serif text-xl text-black mb-4">
                    Neighborhoods Covered
                  </h3>
                  <ul className="space-y-2">
                    {ae.neighborhoods.map((n) => (
                      <li key={n} className="flex items-center text-gray-700">
                        <span className="w-1 h-1 bg-[#D4AF37] mr-3" />
                        {n}
                      </li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h3 className="font-serif text-xl text-black mb-4">
                    Zip Codes Served
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {area.zipCodes.map((z) => (
                      <span
                        key={z}
                        className="px-3 py-1 bg-white border border-gray-200 text-gray-700 text-sm font-mono"
                      >
                        {z}
                      </span>
                    ))}
                  </div>
                  <h3 className="font-serif text-xl text-black mt-8 mb-4">
                    Local Landmarks
                  </h3>
                  <ul className="space-y-2">
                    {ae.landmarks.slice(0, 4).map((l) => (
                      <li key={l} className="flex items-center text-gray-700">
                        <span className="w-1 h-1 bg-[#D4AF37] mr-3" />
                        {l}
                      </li>
                    ))}
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* Reviews */}
        <section className="py-24 lg:py-32 bg-white">
          <div className="container mx-auto px-6">
            <div className="flex flex-col items-center mb-8">
              <div className="flex items-center gap-1 mb-3" aria-label="4.9 out of 5 stars">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className="w-6 h-6 text-[#D4AF37]"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              <p className="font-serif text-2xl md:text-3xl text-black">
                <span className="font-medium">4.9</span>
                <span className="text-gray-400 text-xl"> / 5.0</span>
              </p>
              <p className="text-gray-500 text-sm mt-1">
                Averaged across 127+ verified reviews
              </p>
            </div>
            <SectionHeading
              subtitle="Client Reviews"
              title={
                cityHits.length > 0
                  ? `What ${area.name} Homeowners Say`
                  : `What Homeowners Across ${area.county} County Say`
              }
              align="center"
            />
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16">
              {(cityHits.length > 0 ? cityHits : testimonials.slice(0, 3))
                .slice(0, 3)
                .map((t) => (
                  <TestimonialCard key={t.id} testimonial={t} variant="default" />
                ))}
            </div>
            <div className="flex justify-center mt-12">
              <Link
                href="/our-reviews"
                className="inline-flex items-center text-sm uppercase tracking-[0.2em] text-gray-700 hover:text-[#D4AF37] transition-colors"
              >
                View All Customer Reviews
                <svg
                  className="w-4 h-4 ml-2"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M17 8l4 4m0 0l-4 4m4-4H3"
                  />
                </svg>
              </Link>
            </div>
          </div>
        </section>

        {/* FAQs */}
        <section className="py-24 lg:py-32 bg-gray-50">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              <div>
                <SectionHeading
                  subtitle="Common Questions"
                  title={`${service.name} in ${area.name} FAQs`}
                  align="left"
                />
                <p className="text-gray-600 mt-6 leading-relaxed">
                  Answers to the questions we hear most from {area.name}{' '}
                  homeowners about {serviceShort}.
                </p>
                <div className="mt-8">
                  <CTAButton
                    href={`tel:${businessInfo.phone.replace(/[^0-9]/g, '')}`}
                    variant="primary"
                    size="lg"
                    icon="phone"
                  >
                    Call {businessInfo.phone}
                  </CTAButton>
                </div>
              </div>
              <div>
                <FAQAccordion faqs={faqs} variant="default" />
              </div>
            </div>
          </div>
        </section>

        {/* Other services in this area */}
        <section className="py-20 bg-white border-y border-gray-100">
          <div className="container mx-auto px-6">
            <SectionHeading
              subtitle="Other Repairs"
              title={`Other Service Categories We Cover in ${area.name}`}
              align="center"
            />
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4 mt-12 max-w-5xl mx-auto">
              {services
                .filter((s) => s.slug !== service.slug)
                .map((s) => (
                  <Link
                    key={s.id}
                    href={`/services/${s.slug}`}
                    className="text-center p-4 border border-gray-200 hover:border-[#D4AF37] transition-colors text-sm text-gray-700 hover:text-[#D4AF37]"
                  >
                    {s.name}
                  </Link>
                ))}
            </div>
          </div>
        </section>

        {/* Same service, nearby areas */}
        {nearbySameService.length > 0 && (
          <section className="py-24 lg:py-32 bg-gray-50">
            <div className="container mx-auto px-6">
              <SectionHeading
                subtitle="Nearby Service"
                title={`${service.name} in Nearby Areas`}
                align="center"
              />
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16 max-w-5xl mx-auto">
                {nearbySameService.map((c) => {
                  const a = getAreaForServiceCombo(c);
                  if (!a) return null;
                  return (
                    <Link
                      key={c.slug}
                      href={`/${c.slug}`}
                      className="group p-6 bg-white border border-gray-100 hover:border-[#D4AF37] transition-all duration-300 hover:shadow-lg"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="font-serif text-lg text-black group-hover:text-[#D4AF37] transition-colors">
                            {service.name} in {a.name}
                          </h3>
                          <p className="text-gray-500 text-sm mt-1">
                            {a.county} County · {a.zipCodes[0]}
                          </p>
                        </div>
                        <svg
                          className="w-5 h-5 text-[#D4AF37] transform group-hover:translate-x-1 transition-transform"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M17 8l4 4m0 0l-4 4m4-4H3"
                          />
                        </svg>
                      </div>
                    </Link>
                  );
                })}
              </div>
            </div>
          </section>
        )}

        {/* Final CTA */}
        <section className="py-24 lg:py-32 bg-black">
          <div className="container mx-auto px-6">
            <div className="max-w-3xl mx-auto text-center">
              <div className="w-16 h-px bg-[#D4AF37] mx-auto mb-8" />
              <span className="text-[#D4AF37] text-xs uppercase tracking-[0.3em] font-light mb-4 block">
                {service.name} · {area.name}, NJ
              </span>
              <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-white font-light leading-tight mb-6">
                Ready to Schedule Your {service.name}?
              </h2>
              <p className="text-white/70 text-lg leading-relaxed mb-10">
                Same-day and next-day service available across {area.name} and
                surrounding {area.county} County.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <CTAButton
                  href={`tel:${businessInfo.phone.replace(/[^0-9]/g, '')}`}
                  variant="primary"
                  size="lg"
                  icon="phone"
                >
                  Call {businessInfo.phone}
                </CTAButton>
                <CTAButton
                  href="/contact"
                  variant="outline"
                  size="lg"
                  icon="arrow"
                >
                  Schedule Online
                </CTAButton>
              </div>
            </div>
          </div>
        </section>
      </main>
      <Footer />
    </>
  );
};

export default ServiceAreaPage;
