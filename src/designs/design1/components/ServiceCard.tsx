...
const ServiceIcons: Record<string, React.FC<{ className?: string }>> = {
  ...
  // Range = oven body + cooktop burners on top. Distinct from `oven` (built-in)
  // and `cooktop` (countertop). Earlier this key was missing and the Range
  // service card silently fell back to the oven icon.
  range: ({ className }) => <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="3" y="2" width="18" height="20" rx="2" strokeWidth="1.5"/><circle cx="7" cy="5" r="1" strokeWidth="1.5"/><circle cx="12" cy="5" r="1" strokeWidth="1.5"/><circle cx="17" cy="5" r="1" strokeWidth="1.5"/><line x1="3" y1="9" x2="21" y2="9" strokeWidth="1.5"/><rect x="6" y="12" width="12" height="8" rx="1" strokeWidth="1.5"/><circle cx="9" cy="16" r="1" strokeWidth="1.5"/><circle cx="15" cy="16" r="1" strokeWidth="1.5"/></svg>,
};
